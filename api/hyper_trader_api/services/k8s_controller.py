...
class KubernetesTraderController:
    ...
    def __init__(self):
        ...
        if not settings.k8s_enabled:
            raise RuntimeError("Kubernetes is disabled. Set K8S_ENABLED=true to use K8s features.")
    ...
    def deploy_trader(self, trader: Trader) -> dict[str, Any]:
        ...
        try:
            ...
            raise KubernetesControllerError(f"Deployment failed: {e.reason}") from e
    ...
    def remove_trader(self, trader: Trader) -> None:
        ...
        try:
            ...
            if e.status != 404:
                raise KubernetesControllerError(f"Removal failed: {e.reason}") from e
    ...
    def update_trader_config(self, trader: Trader) -> None:
        ...
        try:
            ...
            raise KubernetesControllerError(f"Config update failed: {e.reason}") from e
    ...
    def restart_trader(self, trader: Trader) -> None:
        ...
        try:
            pods = self.core_api.list_namespaced_pod(
                ...
                label_selector=f"trader-address={trader.wallet_address}",
            )
            ...
            if e.status != 404:
                raise KubernetesControllerError(f"Restart failed: {e.reason}") from e
    ...
    def get_trader_status(self, trader: Trader) -> dict[str, Any]:
        ...
        try:
            ...
            if e.status == 404:
                ...
                raise KubernetesControllerError(f"Status check failed: {e.reason}") from e

        return result

    def get_trader_logs(self, trader: Trader, tail_lines: int = 100) -> str | None:
        """
        ...
        try:
            ...
            if e.status == 404:
                return None
            raise KubernetesControllerError(f"Log retrieval failed: {e.reason}") from e

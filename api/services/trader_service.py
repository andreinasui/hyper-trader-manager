...
from api.models import Trader, TraderConfig, TraderSecret
from api.schemas.trader import TraderCreate, TraderUpdate
from api.services.k8s_controller import (
    KubernetesControllerError,
    KubernetesTraderController,
)
from api.utils.crypto import encrypt_private_key
...
class TraderService:
    ...
    def create_trader(self, user_id: uuid.UUID, trader_data: TraderCreate) -> Trader:
        """
        Create a new trader.
        ...
        trader = Trader(
            ...
            k8s_name=k8s_name,
            status="pending",
            image_tag=trader_data.image_tag or "latest",
        )
        self.db.add(trader)
        ...
        # Encrypt and store private key in DB
        encrypted_key = encrypt_private_key(trader_data.private_key)
        trader_secret = TraderSecret(
            trader_id=trader.id,
            encrypted_private_key=encrypted_key,
            encryption_key_id="primary",  # Reference to current encryption key
        )
        ...
        if self.settings.k8s_enabled and self.k8s:
            try:
                # Deploy to Kubernetes using raw private key
                self.k8s.deploy_trader(trader, trader_data.private_key)
                ...
                raise TraderServiceError(f"Deployment failed: {e}")
        else:
    ...
    def list_traders(self, user_id: uuid.UUID) -> List[Trader]:
        """
    ...
    def get_trader(self, trader_id: uuid.UUID, user_id: uuid.UUID) -> Trader:
        """
    ...
    def update_trader(
        self, trader_id: uuid.UUID, user_id: uuid.UUID, update_data: TraderUpdate
    ) -> Trader:
        """
        ...
        if self.settings.k8s_enabled and self.k8s:
            try:
                ...
                raise TraderServiceError(f"Config update failed: {e}")
        else:
    ...
    def delete_trader(self, trader_id: uuid.UUID, user_id: uuid.UUID) -> None:
        """
        Delete a trader.
        ...
        if self.settings.k8s_enabled and self.k8s:
            try:
                ...
                raise TraderServiceError(f"Deletion failed: {e}")
        else:
    ...
    def restart_trader(self, trader_id: uuid.UUID, user_id: uuid.UUID) -> None:
        """
        ...
        try:
            ...
            raise TraderServiceError(f"Restart failed: {e}")

    def get_trader_status(self, trader_id: uuid.UUID, user_id: uuid.UUID) -> Dict[str, Any]:
        """
    ...
    def get_trader_logs(
        self, trader_id: uuid.UUID, user_id: uuid.UUID, tail_lines: int = 100
    ) -> str:
        """

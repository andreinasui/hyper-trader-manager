...
class TestRegister:
    ...
    @patch("hyper_trader_api.routers.auth.AuthService.register_user")
    @patch("hyper_trader_api.routers.auth.JWTService.create_access_token")
    @patch("hyper_trader_api.routers.auth.JWTService.create_refresh_token")
    ...
    def test_register_with_api_key_backward_compatibility(self, mock_register, client):
        ...
        response = client.post("/api/v1/auth/register", json={"email": "legacy@example.com"})
...
class TestLogin:
    ...
    @patch("hyper_trader_api.routers.auth.AuthService.authenticate_user")
    @patch("hyper_trader_api.routers.auth.JWTService.create_access_token")
    @patch("hyper_trader_api.routers.auth.JWTService.create_refresh_token")
...
class TestRefresh:
    ...
    @patch("hyper_trader_api.routers.auth.JWTService.verify_refresh_token")
    @patch("hyper_trader_api.routers.auth.JWTService.create_access_token")
...
class TestGetMe:
    ...
    @patch("hyper_trader_api.middleware.jwt_auth.get_current_user_from_jwt")
    @patch("hyper_trader_api.middleware.jwt_auth.get_current_user_from_api_key")
...
class TestLogout:
    ...
    @patch("hyper_trader_api.middleware.jwt_auth.get_current_user_from_jwt")
    @patch("hyper_trader_api.routers.auth.JWTService.revoke_refresh_token")
